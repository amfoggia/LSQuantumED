...
TEST(DSF, chain1D) {
  ...
  Basis b1{env,1};
  Hamiltonian<chain1D> h1{env,b1,lat,J1,Delta1,J2,Delta2};
  ierr = h1.build_diag(env); ASSERT_EQ(0,ierr);
  ierr = h1.build_off_diag(env); ASSERT_EQ(0,ierr);
  MatView(h1.hamilt, PETSC_VIEWER_STDOUT_WORLD);
  ...
  ierr = Phys::DynStructFactor<chain1D,Sqp>(env, dsf_data, real_eig, xr, 0);
